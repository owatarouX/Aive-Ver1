...
void CEnemy::HitCheckPlayer()
{
	...
	//x======ボス本体とプレイヤーの体の当たり判定======x
	//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
	int knockback = 80;	//ノックバックの値

	// ①プレイヤーの向き参照の判定 ●「switch (player ->GetDirection()) 」
	...
	//		{
	//		case 0:	// プレイヤーが上を向いていたら
	//			player->SetPosY(m_bossList.GetPosY() - BOSS_SIZE::DOWN - knockback);	// ボスの上部に出現
	//			player->SetMovevalY(0);					// 移動量を0に
	//			player->SetDamage(POWER::ENEMY_PHYSICAL);	// プレイヤーのHP減少
	//			player->SetHitFlg();	// 当たっている状態にする(無敵処理を呼び出すため)
	//			break;
	//		case 1:	// プレイヤーが下を向いていたら
	//			player->SetPosY(m_bossList.GetPosY() + BOSS_SIZE::TOP + knockback);	// ボスの上部に出現
	//			player->SetMovevalY(0);					// 移動量を0に
	//			player->SetDamage(POWER::ENEMY_PHYSICAL);	// プレイヤーのHP減少
	//			player->SetHitFlg();	// 当たっている状態にする(無敵処理を呼び出すため)
	//			break;
	//		case 2:	// プレイヤーが左を向いていたら
	//			player->SetPosX(m_bossList.GetPosX() - BOSS_SIZE::LEFT - knockback);	// ボスの上部に出現
	//			player->SetMovevalX(0);					// 移動量を0に
	//			player->SetDamage(POWER::ENEMY_PHYSICAL);	// プレイヤーのHP減少
	//			player->SetHitFlg();	// 当たっている状態にする(無敵処理を呼び出すため)
	//			break;
	//		case 3:	// プレイヤーが右を向いていたら
	//			player->SetPosX(m_bossList.GetPosX() + BOSS_SIZE::RIGHT + knockback);	// ボスの上部に出現
	//			player->SetMovevalX(0);					// 移動量を0に
	//			player->SetDamage(POWER::ENEMY_PHYSICAL);	// プレイヤーのHP減少
	//			player->SetHitFlg();	// 当たっている状態にする(無敵処理を呼び出すため)
	//			break;
